...
func main() {
	...
	// add CF credentials to errand config in the release
	// check if the broker has maintenance info => Template could pass a flag "supportsMaintenanceInfo"
	// check if we can use CF => by checking if the errandConfig has cf credentials && CF OSBAPI version > 2.15
